...
export default function UniformRequestStatusView({ isDispatchAdmin = false }: Props) {
  ...
  return (
    <section className="rounded-lg border bg-white p-6 shadow-sm">
      ...
      {isDispatchAdmin && !isLoading && !error && visibleRequests.length > 0 && (
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {visibleRequests.map((request) => (
                <tr
                  key={request.id}
                  onClick={isDispatchAdmin ? () => setSelectedId(request.id) : undefined}
                  className={isDispatchAdmin ? "cursor-pointer transition hover:bg-indigo-50/60" : undefined}
                >
